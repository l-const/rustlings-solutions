...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if (tuple.0 >= 0 && tuple.0 <= 255) &&(tuple.1 >= 0 && tuple.1 <= 255) && (tuple.2 >= 0 && tuple.2 <= 255) {
           return Ok(Color{ red : tuple.0 as u8,
                   green: tuple.1 as u8,
                   blue: tuple.2 as u8,
               })
           } else {
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if (arr[0] >= 0 && arr[0] <= 255) &&(arr[1] >= 0 && arr[1] <= 255) && (arr[2] >= 0 && arr[2] <= 255) {
         return Ok(Color{ red : arr[0] as u8,
                green: arr[1] as u8,
                blue: arr[2] as u8,
            })
        } else {
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if  slice.len() == 3 {
            ...
            return  Ok(Color{ red : slice[0] as u8,
                    green: slice[1] as u8,
                    blue: slice[2] as u8,
                })
            } else {
           ...
           return  Err(String::from("Not enough length!"))
        }
    }
}
...
fn main() {
    ...
    let c1 = Color::try_from((183, 65, 14)).is_err();
    // println!("{:?}", c1);
